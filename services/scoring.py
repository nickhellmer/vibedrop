...
def score_v4():
    ...
    for user in users:
        ...
        num_cycles = len(set([s.cycle_date for s in submissions]))
        ...
        # Bayesian Smoothing: like rate w/ shrinkage
        smoothed = ((total_likes + alpha * mu) / (total_ratings + alpha)) * 10

        # Participation bonus: capped to one full point if active every week
        if circle_members == 1:
            participation_bonus = 0
        else:
            participation_bonus = beta * min(num_cycles / 10, 1)

        score = round(smoothed + participation_bonus, 2)

        if TESTING_MODE:
            print(f"[SCORING v4] {user.vibedrop_username}: Likes={total_likes}, Rated={total_ratings}, Cycles={num_cycles}, Score={score}")

        # Update or insert DropCred
